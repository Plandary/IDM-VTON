...
parsing_model = Parsing(6)
openpose_model = OpenPose(6)
        ...
        with torch.cuda.amp.autocast():
            with torch.no_grad():
                ...
                with torch.inference_mode():
                    ...
                    pose_img = tensor_transform(pose_img).unsqueeze(
                        0).to(device, torch.float16)
                    garm_tensor = tensor_transform(garm_img).unsqueeze(
                        0).to(device, torch.float16)
...
with image_blocks as demo:
    gr.Markdown("## Plandary(All Rights Reserved) Virtal Try-on 👕👔👚")
    gr.Markdown("Virtual Try-on with your image and garment image.")
    with gr.Row():
        with gr.Column():
            imgs = gr.ImageEditor(sources='upload', type="pil",
                                  label='Human. Mask with pen or use auto-masking', interactive=True)
            with gr.Row():
        ...
        with gr.Column():
            ...
            with gr.Row(elem_id="prompt-container"):
                with gr.Row():
                    prompt = gr.Textbox(
                        placeholder="Description of garment ex) Short Sleeve Round Neck T-shirts", show_label=False, elem_id="prompt")
            example = gr.Examples(
                inputs=garm_img,
                examples_per_page=8,
    ...
    with gr.Column():
        ...
        with gr.Accordion(label="Advanced Settings", open=False):
            with gr.Row():
                ...
                seed = gr.Number(label="Seed", minimum=-1,
                                 maximum=2147483647, step=1, value=42)

    try_button.click(fn=start_tryon, inputs=[imgs, garm_img, prompt, is_checked, is_checked_crop, denoise_steps, seed], outputs=[
                     image_out, masked_img], api_name='tryon')
